import type { NewsItem } from "../types";

export const news: NewsItem[] = [
  {
    id: 1,
    slug: "vitoria-lusitano",
    title: "Brasil supera Lusitano BX por 3 a 1 em casa",
    category: "Resultado",
    excerpt:
      "Com gols de Pretaum, Jackson e Kaik, a equipe se mantém viva na briga pelo G-4 da Liga Trabalhista.",
    date: "2026-05-10",
    image: "/images/news/vitoria-lusitano.webp",
    featured: true,
    content:
      "O Brasil voltou a vencer em casa após uma sequência de duas rodadas sem triunfos. O time entrou em campo no Stade Roi Baudouin com postura ofensiva e abriu o placar logo aos 12 minutos do primeiro tempo. Pretaum, em grande fase, marcou o seu nono gol no campeonato. Na etapa final, Jackson ampliou de cabeça após cobrança de escanteio e Kaik fechou a conta nos minutos finais. O resultado mantém o clube vivo na briga pelo G-4 da Liga Trabalhista.",
  },
  {
    id: 2,
    slug: "novo-uniforme",
    title: "Clube apresenta novo uniforme da temporada",
    category: "Clube",
    excerpt:
      "Modelo combina o azul de Bruxelas com o amarelo da bandeira brasileira — confeccionado em parceria com fornecedor local.",
    date: "2026-05-08",
    image: "/images/news/uniforme.webp",
    content:
      "Em evento na sede do clube, o Brasil revelou o uniforme oficial da temporada 2025/26. A camisa principal mantém o tradicional azul-marinho do clube, com detalhes em amarelo que remetem à bandeira brasileira. O uniforme reserva, na cor branca com listras verticais, homenageia as cores tradicionais belgas. A parceria com o fornecedor local fortalece o vínculo do clube com a comunidade.",
  },
  {
    id: 3,
    slug: "treino-aberto",
    title: "Treino aberto à comunidade reúne torcedores em Anderlecht",
    category: "Comunidade",
    excerpt:
      "Mais de 200 brasileiros compareceram ao Stade Communal para acompanhar a preparação do elenco.",
    date: "2026-05-04",
    image: "/images/news/treino-aberto.webp",
    content:
      "O treino aberto realizado neste domingo no Stade Communal de Anderlecht reuniu mais de 200 torcedores brasileiros residentes em Bruxelas. A iniciativa, que já se tornou tradição, aproxima a comunidade do dia a dia do clube e fortalece os laços com os atletas. Houve sessão de autógrafos, fotos e até um pequeno torneio entre crianças no campo auxiliar.",
  },
  {
    id: 4,
    slug: "parceria-comercial",
    title: "Clube anuncia nova parceria comercial",
    category: "Patrocínio",
    excerpt:
      "Acordo de dois anos fortalece a estrutura do Brasil e abre novas frentes de captação.",
    date: "2026-04-28",
    image: "/images/news/parceria.webp",
    content:
      "A diretoria do Brasil oficializou um acordo de dois anos com um novo parceiro comercial. A negociação garante o suporte financeiro para a temporada 2025/26 e abre frentes adicionais de captação para o próximo ciclo. Os detalhes do contrato não foram divulgados, mas o presidente Lins destacou que a parceria é estratégica para o crescimento da estrutura do clube.",
  },
  {
    id: 5,
    slug: "pretaum-artilheiro",
    title: "Pretaum lidera artilharia da Liga Trabalhista",
    category: "Destaque",
    excerpt:
      "Com 9 gols em 7 jogos, o atacante é o nome do momento na competição amadora belga.",
    date: "2026-04-21",
    image: "/images/news/pretaum-artilheiro.webp",
    content:
      "O atacante Pretaum chegou a 9 gols em apenas 7 jogos disputados e assumiu a liderança isolada da artilharia da Liga Trabalhista. O jogador, conhecido pela rapidez e precisão na finalização, vem sendo decisivo em momentos importantes do campeonato. A comissão técnica destaca a evolução tática e o engajamento do atleta dentro e fora de campo.",
  },
  {
    id: 6,
    slug: "categoria-base",
    title: "Brasil avalia criação de categoria de base",
    category: "Clube",
    excerpt:
      "Diretoria estuda formato para crianças e adolescentes brasileiros em Bruxelas a partir de 2026/27.",
    date: "2026-04-14",
    image: "/images/news/base.webp",
    content:
      "Em reunião com membros da comunidade brasileira, a diretoria apresentou os primeiros estudos para a criação de uma categoria de base do clube. A iniciativa busca acolher crianças e adolescentes filhos de imigrantes brasileiros em Bruxelas, oferecendo formação esportiva e socialização através do futebol. O projeto deve sair do papel na temporada 2026/27.",
  },
];
